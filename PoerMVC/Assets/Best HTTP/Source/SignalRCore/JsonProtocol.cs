...
namespace BestHTTP.SignalRCore
{
    ...
    public sealed class JsonProtocol : IProtocol
    {
        ...
        public void ParseMessages(string data, ref List<Message> messages)
        {
            ...
            if (separatorIdx == -1)
                throw new Exception(string.Format("Missing separator in data: '{0}'", data));
        ...
        public void ParseMessages(byte[] data, ref List<Message> messages) {
            ParseMessages(System.Text.Encoding.UTF8.GetString(data, 0, data.Length), ref messages);
        }
        ...
        public BufferSegment EncodeMessage(Message message)
        {
            ...
            return !string.IsNullOrEmpty(json) ? JsonProtocol.WithSeparator(json) : BufferSegment.Empty;
        }
